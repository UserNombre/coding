...
def print_debug_info(lab_map, guard_position, guard_direction):
    ...
    row_slice = slice(max(0, guard_position[0] - height//2), min(len(lab_map), guard_position[0] + height//2))
    column_slice = slice(max(0, guard_position[1] - width//2), min(len(lab_map[0]), guard_position[1] + width//2))
    sys.stdout.write("\033[H")
    print(f"POS: {guard_position}, DIR: {guard_direction}".ljust(width))
    print("\n".join(bytes(row[column_slice]).decode() for row in lab_map[row_slice]))
